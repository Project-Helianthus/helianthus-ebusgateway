...
| Tesla Gen3 WC3 24.44.3 current-limit SemReg projection | A stateful, injected-record-only `helianthus.pack.evse@1.0.0` publication may expose `semantic.v1.evse.current.get`, mTLS `SemanticEVSECurrent`, and the configured read-only Portal EVSE BFF. `evse.limit.configured_current` is published only from valid persistent terminal evidence. `evse.limit.allocated_current` is published only from correlated provisional set/ACK/readback evidence with a finite 1..86399 second timeout and `InhibitCharging=false`. | The native `modbus.v1.tesla.gen3.evse.current_limit.get` tool remains evidence-only. Missing, malformed, expired, inhibited, zero-timeout, or correlation-mismatched provisional evidence withholds only allocated current with an explicit projection loss; valid configured current remains. The Portal BFF forwards only the fixed GraphQL response; no gateway acquisition, write, operation capability, Prometheus metric, alias, fallback, shadow, or dual publication is created by this injected seam. |
