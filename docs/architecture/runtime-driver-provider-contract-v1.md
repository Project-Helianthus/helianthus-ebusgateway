...
| Tesla Gen3 WC3 24.44.3 current-limit SemReg projection | A stateful, injected-record-only `helianthus.pack.evse@1.0.0` publication may expose `semantic.v1.evse.current.get` and mTLS `SemanticEVSECurrent`. `evse.limit.configured_current` is published only from valid persistent terminal evidence. `evse.limit.allocated_current` is published only from correlated provisional set/ACK/readback evidence with a finite 1..86399 second timeout and `InhibitCharging=false`. | The native `modbus.v1.tesla.gen3.evse.current_limit.get` tool remains evidence-only. Evidence digests serialize the version, decoded values, every native payload, and captured lifecycle coordinates. The first accepted sequence is `1`; later distinct input must advance by exactly one. An identical accepted `(generation, sequence, input digest)` retry is a no-op; a distinct input at the same sequence and every gap are rejected without mutation. Qualified available read capabilities carry the accepted immutable persistent record evidence and, only when valid, the bounded provisional record evidence; source identity alone never activates them. Freshness and allocation expiry use the injected independent monotonic clock; wall timestamps are reporting data only. An equal wall observation/evaluation may share its receipt monotonic coordinate; a delayed evaluation requires an explicit non-regressing evaluation monotonic coordinate. A newer accepted bundle that cannot promote allocation withdraws the prior allocated candidate from SemReg while retaining configured current; its stable candidate revision high-water is retained so later reactivation advances rather than reuses history. Same-epoch receipt/evaluation coordinates may not regress below the retained publication/read floor. Missing, malformed, expired, inhibited, zero-timeout, or correlation-mismatched provisional evidence withholds only allocated current with an explicit projection loss; valid configured current remains. No gateway configuration, listener, acquisition, write, operation capability, Portal view, Prometheus metric, alias, fallback, shadow, or dual publication is created by this injected seam. |
