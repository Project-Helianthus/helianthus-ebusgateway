...
| Tesla Gen3 WC3 24.44.3 current-limit SemReg projection | A stateful, injected-record-only `helianthus.pack.evse@1.0.0` publication may expose `semantic.v1.evse.current.get` and mTLS `SemanticEVSECurrent`. `evse.limit.configured_current` is published only from valid persistent terminal evidence. `evse.limit.allocated_current` is published only from correlated provisional set/ACK/readback evidence with a finite 1..86399 second timeout and `InhibitCharging=false`. | The native `modbus.v1.tesla.gen3.evse.current_limit.get` tool remains evidence-only. Qualified available read capabilities carry the accepted immutable persistent record evidence and, only when valid, the bounded provisional record evidence; source identity alone never activates them. Missing, malformed, expired, inhibited, zero-timeout, or correlation-mismatched provisional evidence withholds only allocated current with an explicit projection loss; valid configured current remains. No gateway configuration, listener, acquisition, write, operation capability, Portal view, Prometheus metric, alias, fallback, shadow, or dual publication is created by this injected seam. |
