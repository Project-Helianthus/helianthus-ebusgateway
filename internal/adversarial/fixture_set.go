...
func ValidateFixtureProjectionV1(r ReportV1, b BoundFixture) error {
	...
	startedAt, ok := validStamp(r.Execution.StartedAt)
	if !ok {
		return fmt.Errorf("%w: projection_time", ErrInvalidReport)
	}
	producer := producerIdentity{Repository: r.Provenance.Producer.Repository, Commit: r.Provenance.Producer.Commit, Component: r.Provenance.Producer.Component, BuildKind: r.Provenance.Producer.BuildKind, BuildSHA256: r.Provenance.Producer.BuildSHA256, InputGatewayReportSHA256: cloneString(r.Provenance.Producer.InputGatewayReportSHA256)}
	expected, err := (Executor{StartedAt: startedAt, producer: producer, subject: r.Provenance.Subject.Commit}).runBound(b)
	if err != nil {
